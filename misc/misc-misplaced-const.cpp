...
//
//This check diagnoses when a const qualifier is applied to a typedef/ using to a pointer type rather than to the
// pointee, because such constructs are often misleading to developers because the const applies to the pointer rather
// than the pointee.
//
...
namespace misc::misplaced_const {
    ...
    void check() {
        std::cout << "-- check misc-misplaced-const" << std::endl;
    }
} // namespace misc::misplaced_const
