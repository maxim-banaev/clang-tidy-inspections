...
// While the issue is obvious in this former example, it might not be so in the following:
// void MyClass::f(double d) { int_member_ += d; }.
//
// This rule is part of the “Expressions and statements” profile of the C++ Core Guidelines, corresponding to rule ES.46.
// See
//
...
// a floating-point to a narrower floating-point (e.g. double to float) if WarnOnFloatingPointNarrowingConversion
// Option is set.
// This check will flag:
// All narrowing conversions that are not marked by an explicit cast (c-style or static_cast). For example:
